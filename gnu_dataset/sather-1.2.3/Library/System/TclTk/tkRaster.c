	     ...
	     int hgt) 
{
    ...
    newpm = XCreatePixmap (RasterPtr->display, 
...
DestroyRaster(clientData)
    char *clientData;	/* Info about Raster widget. */
{
    Raster *RasterPtr = (Raster *) clientData;
    ClientData * dataPtr;
    Tcl_HashSearch search;
		...
		int index) 
{
    ...
    printf("SetDrawEnviron\n");
    if (DrawEnvIndex (interp, RasterPtr, index, 
		      (ClientData*)&drawEnvPtr) != TCL_OK){
	printf("ERROR: failed DrawEnvIndex\n");
	return -1;
    }
    result = SetDrawEnv (interp, RasterPtr, drawEnvPtr);
    return result;
		 ...
		 int index) 
{
    ...
    printf("GetRasterColour\n");
    if (DrawEnvIndex (interp, RasterPtr, index, 
		      (ClientData*)&drawEnvPtr) != TCL_OK){
	printf("ERROR: failed DrawEnvIndex\n");
    }
    result = SetDrawEnv (interp, RasterPtr, drawEnvPtr);
    printf("blue %d \n", drawEnvPtr->fgColor->blue);
...
void
RasterClear(Tk_Raster *Tk_RasterPtr)
{
  Raster *RasterPtr = (Raster *)Tk_RasterPtr;
  XFillRectangle (RasterPtr->display, RasterPtr->pm, RasterPtr->copyGC,
		  0, 0, RasterPtr->width, RasterPtr->height);
}

void
RasterDisplay(Tk_Raster *RasterPtr)
{
  arrangeDisplay (RasterPtr, LOW, LOW, HIGH, HIGH);
}
