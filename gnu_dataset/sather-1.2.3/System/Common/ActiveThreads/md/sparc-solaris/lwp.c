 ...
 * We use a system reserved global register g7 to hold it 
 * Conveniently enough, it is reserved by the SPARC architecture just
 * for this purpose
 */
...
static int procs(void){
  ...
  /* 
   * Check if the overriding env. variable is present
   * If so, set the number of processors to one and do not 
   * lwps to processors. Some of the cache simulators (SHADE)
   * need this because they cannot simulate the right system calls
   */
     
  if (getenv("AT_SHADE")!=NULL){
    ncpu=1;
  }
  else {
    /*at_printf("sysconf\n");*/
