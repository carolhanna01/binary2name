...
/*$RCSfile: spprint.c,v $ $Revision: 1.2.2.2 $ */
...
private const char *
stream_printf_scan(stream * s, const char *format)
{
...
{
    const char *fp = stream_printf_scan(s, format);
    char str[25];
    ...
    sprintf(str, "%d", v);
    stream_puts(s, str);
    return stream_printf_scan(s, fp + 2);
}
const char *
...
pprintld1(stream * s, const char *format, long v)
{
    ...
    sprintf(str, "%ld", v);
    stream_puts(s, str);
    return stream_printf_scan(s, fp + 3);
}
const char *
