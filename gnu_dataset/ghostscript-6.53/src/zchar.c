...
/*$RCSfile: zchar.c,v $ $Revision: 1.3.2.3 $ */
/* Character operators */
#include "ghost.h"
#include "oper.h"
#include "gsstruct.h"
		     ...
		     op_proc_t endproc /* end procedure */ )
{
	...
	/* If a show operator with changing the currentfont is run,
	   we don't have to raise a rangecheck error, per the Red Book. 
	   We check the currentfont is changed in the proc of a cshow here. */
