...
/*$Id: gdevdj9.c,v 1.4.2.1 2001/06/05 07:07:37 raph Exp $*/
...
const gx_device_cdj970 gs_cdj970_device =
				...
				PRESENTATION, PLAIN_PAPER, NONE, 4, DJ970C, 2,
...
{				
	...
	if (retCode < 0)
		...
		if (retCode < 0)
			return (retCode);
		else if (cdj970->ptype == DJ970C) {
			/* write header information (PCL3GUI Language) */
			gx_device_printer *const ppdev = (gx_device_printer *)pdev;
					
			cdj970_write_header (pdev, ppdev->file);
		 }
	}
...
private int cdj970_write_header (gx_device *pdev, FILE * prn_stream)
{
	...
	/* enter duplex mode / reverse enineering */
