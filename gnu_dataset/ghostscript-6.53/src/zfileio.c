...
/*$RCSfile: zfileio.c,v $ $Revision: 1.9.2.4 $ */
/* File I/O operators */
#include "memory_.h"
#include "ghost.h"
#include "gp.h"
...
/**
 * handle_read_status: Handle an exceptional status return from a read stream.
 * @i_ctx_p: The interpreter context.
 * @ch: The status return code.
 * @fop: A pointer to the ref for the stream.
 * @pindex: Pointer to additional integer to push on estack.
 * @cont: Continuation procedure.
 *
 * This routine handles exceptional status returns from read streams. If
 * @pindex is non-NULL, the integer it points to is pushed on the estack
 * for the continuation to consume.
 *
 * Return value: 1 on EOF, 0 on other success, o_push_estack if estack
 * needs enlargement, or error.
 **/
private int
...
/**
 * handle_write_status: Handle exceptional status return from a write stream.
 * @i_ctx_p: The interpreter context.
 * @ch: The status return code.
 * @fop: A pointer to the ref for the stream.
 * @pindex: Pointer to additional integer to push on estack.
 * @cont: Continuation procedure.
 *
 * This routine handles exceptional status returns from write streams. If
 * @pindex is non-NULL, the integer it points to is pushed on the estack
 * for the continuation to consume.
 *
 * Return value: 1 on EOF, 0 on other success, o_push_estack if estack
