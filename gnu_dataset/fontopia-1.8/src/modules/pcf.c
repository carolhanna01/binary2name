...
static inline int32_t get_int(unsigned char *d)
{
  int32_t res = 0;
  res = d[0] | ((int32_t)d[1] << 8) | ((int32_t)d[2] << 16) | ((int32_t)d[3] << 24);
  return res;
}
...
static inline u_int16_t get_ushort(unsigned char *d)
{
  u_int16_t res = 0;
  res = d[0] | ((u_int16_t)d[1] << 8);
  return res;
}
...
struct font_s *pcf_load_font_file(char *file_name)
{
	...
	fclose(font_file);
	font = pcf_load_font((unsigned char *)file_data, file_size);
	free(file_data);
	return font;
...
struct font_s *pcf_load_font(unsigned char *file_data, long file_size)
{
	struct font_s *font = (struct font_s *)NULL;
	font = (struct font_s *)malloc(sizeof(struct font_s));
	if(!font) goto memory_error;
...
int get_bitmap_table(char *table_data, struct font_s *font)
{
	...
	int swapbytes = need_swap_bytes(format);
	int swapbits  = 0; //need_swap_bits (format);
	int bcount = table_get_int(st[1], swapbytes, swapbits);
        font->length = bcount;
	...
	switch(padding)
	{
        ...
        int j;
	int size = table_get_int(sizes[padding], swapbytes, swapbits);
	for(j = 0; j < bcount; j++)
	{
		u_int32_t off = (u_int32_t)table_get_int(offsets[j], swapbytes, swapbits);
		...
		{
			u_int32_t off2 = (u_int32_t)table_get_int(offsets[j+1], swapbytes, swapbits);
			sz = off2-off;
		}
		...
		while(k < sz)
		{
			...
			if(line_bytes == 0)
			{
				...
				while(m--)
				{
                                        if(swapbytes) l = (l << 8) | *d;
                                        else
                                        {
                                            unsigned int l2 = reverse_char(*d);
                                            l = l | (l2 << 8);
                                        }
					d++;
					k++;
...
int get_encodings_table(char *table_data, struct font_s *font)
{
	...
	if(swapbytes)
	{
		...
		default_char = swap_uword(default_char & 0xffff);
	}
	...
	if(!min_byte1 && !max_byte1)	// single byte encodings
	{
		...
		{
			j = glyphindex[i];
			if(swapbytes) j = swap_uword(j & 0xffff);
...
int get_swidths_table(char *table_data, struct font_s *font)
{
	...
	int swapbytes = need_swap_bytes(format);
	int swapbits  = 0; //need_swap_bits (format);
	int count = table_get_int(st[1], swapbytes, swapbits);
        font->length = count;
        // make sure we have a charinfo struct
        if(!alloc_font_charinfo(font, 0)) return 0;
        struct char_info_s *char_info = font->char_info;
	...
	for(i = 0; i < count; i++)
	{
		j = table_get_int(swidths[i], swapbytes, swapbits);
...
int pcf_write_to_file(FILE *file, struct font_s *font)
{
    ...
    if(!font->char_info)
    {
        if(!create_char_info(font)) return 0;
    }
    ...
    int enc_len = sizeof(u_int16_t)*((max_char_or_byte2-min_char_or_byte2+1)*(max_byte1-min_byte1+1));
