...
void export_glyphs(struct font_s *font)
{
  char *fn = 0;
  int len = 0;
  if(font_file_name)
  ...
  fprintf(f, "#fontopia glyph file\n");
  fprintf(f, "# This file contains a textual representation of the\n");
  fprintf(f, "# glyphs of (%s).\n", font_file_name);
  fprintf(f, "# This file is informative only, as the actual font file\n");
  fprintf(f, "# may have been modified in fontopia. Currently you can not\n");
  fprintf(f, "# open this file in fontopia for editing, we will try to support\n");
  fprintf(f, "# this function in future versions.\n");
  fprintf(f, "# For now, this file is yours!. You are free to do whatever on heavenly\n");
  fprintf(f, "# earth you want to do with it!.\n");
  fprintf(f, "#\n");
  fprintf(f, "#Some info about your font:\n\n");
  fprintf(f, "Length=%d chars\n", font->length);
  fprintf(f, "Width=%d pixels\n", font->width);
  fprintf(f, "Height=%d pixels\n", font->height);
  fprintf(f, "Charsize=%d bytes\n", font->charsize);
...
int get_glyph_index(struct font_s *font)
{
  ...
  {
    return (right_window.cursor.row*(right_window.width/2) + right_window.cursor.col)*font->charsize;
  }
  else
  {
    return (right_window.cursor.row*right_window.width + right_window.cursor.col)*font->charsize;
  }
  return 0;
  */
}
