	...
	while(1)
	{
		...
		case 'h':	//show help & exit
		  printf("Fontopia for GNU/Linux, Version %s\n"
		    "By Mohammed Isam, 2015, 2016, 2017, 2018\n"
...
void init(int argc, char **argv)
{
  ...
  if(total_height > SCREEN_H-2 ||
     total_width  > SCREEN_W-3)
  {
    restoreTerminal(); 
    setScreenColors(WHITE, BGDEFAULT);
...
void quit(struct font_s *font)
{
  ...
  if(font->state == MODIFIED || font->state == NEW_MODIFIED)
  {
    ...
    if(res == ABORT) return;
    if(res == NO) exit_gracefully();
  }
  buffer_mode_on = 0;
  res = check_font_saved(font, 0);
  if(res == 0) exit_gracefully();
  return;
}
