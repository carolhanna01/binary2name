...
class IntervalCheckBox(gtk.HBox, cfg.ConfigUtils):
    def __init__(self, exname, varname):
        ...
        self.checkbox_dict = {}
        for x in range(1, 17):
            self.checkbox_dict[x] = c = gtk.ToggleButton(const.short_interval_name[x])
    ...
    def on_toggle(self, _o):
        ...
        for k in keys:
            if self.checkbox_dict[k].get_active():
                v.append(k)
...
class MultipleIntervalConfigWidget(gtk.VBox, cfg.ConfigUtils):
    ...
    def __init__(self, exname):
        ...
        self.g_num_int_spin = gu.nSpinButton(self.m_exname,
                       'number_of_intervals',
                       gtk.Adjustment(1, 1, self.MAX_INT, 1, 10))
        ...
        box.pack_start(gtk.Label(
                _("Togglebuttons are for interval number:")), False)
        ...
        for x in range(1, 17):
            self.g_interval_chk[x] = c = gtk.ToggleButton(const.short_interval_name[x])
            c.set_data('interval', x)
            if x in V:
                c.set_active(True)
            c.show()
    ...
    def on_num_int_spin(self, _o):
        adj = gtk.Adjustment(self.get_int('cur_edit_interval'), 1,
                  self.get_int('number_of_intervals'), 1, 10, 10)
