...
guint32 interp_color_1 ( double fraction )
   ...
   ********************************************************************************/
   ...
   index = roundint(fraction*(n_color_spectrum_1 - 1));
   color = color_spectrum_1[index];
...
guint32 interp_color_2 ( double fraction )
   ...
   ********************************************************************************/
   ...
   index = roundint(fraction*(n_color_spectrum_2 - 1));
   color = color_spectrum_2[index];
...
int find_indices ( int index1, int index2, double *x, double xi )
   ...
   ********************************************************************************/
   ...
   if ( x[index1] < x[index2] )
      x1 = bsearch(&xi, &x[index1], npts, sizeof(double), search_compare_ascending);
   else
      x1 = bsearch(&xi, &x[index1], npts, sizeof(double), search_compare_descending);
...
char * get_string ( char *string, unsigned int *i1_str, unsigned int *i2_str,
   ...
   free(string_get);
   string_get = NULL;
   string_get = xmalloc(*size + 1);
   memset(string_get, 0, *size + 1);
   strcpy(string_get, string);


   /* Get string limits */
   i = 0;
   while ( (string_get[i] == ' ' || string_get[i] == '\t') && i < (int) *size )
      i++;

   if ( string_get[i] == '#' || string_get[i] != '\"' )
      return NULL;

   i1 = i + 1;
   if ( (pchar = strchr(&string_get[i1], '\"')) == NULL )
      return NULL;
   *pchar = '\0';
   i2 = i1 + pchar - &string_get[i1] - 1;
   ...
   ************************************************************************/
   if ( flag == 1 )
      for ( i=i1; i<=i2; i++ )
         ...
         if ( string_get[i] == '\\' && string_get[i+1] == '\\' )
            {
            for ( j=i; j<=i2; j++ )
               string_get[j] = string_get[j+1];
            i2--;
            }

          /* Find newline escape sequences \n */
         else if ( string_get[i] == '\\' && string_get[i+1] == 'n' )
            {
            string_get[i] = '\n';
            for ( j=i+1; j<=i2; j++ )
               string_get[j] = string_get[j+1];
         ...
         /* Find Unicode hexadecimal or octal escape sequences */
            ...
            if ( string_get[i+1] == 'x' &&
                 strchr("0123456789abcdefABCDEF", string_get[i+2]) != NULL &&
                 strchr("0123456789abcdefABCDEF", string_get[i+3]) != NULL )
               sscanf(&string_get[i+2], "%2x", &strvalue);

            else if ( strchr("01234567", string_get[i+1]) != NULL &&
                      strchr("01234567", string_get[i+2]) != NULL &&
                      strchr("01234567", string_get[i+3]) != NULL )
               sscanf(&string_get[i+1], "%3o", &strvalue);
            ...
            /* Modify string */
            string_get[i] = strvalue;
            for ( j=i+1; j<=i2-3; j++ )
               string_get[j] = string_get[j+3];
            i2 = i2 - 3;
            }
         }
   string_get[i2+1] = '\0';
...
void put_pixel ( GdkPixbuf *pixbuf, int i, int j, guint32 color )
   ...
   pixels     = gdk_pixbuf_get_pixels(pixbuf);       /* pointer to pixel data of pixbuf */
   n_channels = gdk_pixbuf_get_n_channels(pixbuf);   /* samples per pixel; n_channels = 4 */
   rowstride  = gdk_pixbuf_get_rowstride(pixbuf);    /* bytes between start of one row and start of next row */
