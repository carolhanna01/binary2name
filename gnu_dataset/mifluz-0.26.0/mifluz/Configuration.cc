...
//
// $Id: Configuration.cc,v 1.11 2001/06/29 14:14:08 loic Exp $
//
...
Configuration::Configuration(const Configuration& other) : Object()
{
  ...
  const char* key;
  for(other.dcGlobalVars.Start_Get(cursor); (key = other.dcGlobalVars.Get_Next(cursor));) {
    Add(key, other[key]);
  }
}
...
#define CONFIG_BUFFER_SIZE (50*1024) 
 	...
 	if (strcasecmp((char*)name, "include") == 0)
 	{
