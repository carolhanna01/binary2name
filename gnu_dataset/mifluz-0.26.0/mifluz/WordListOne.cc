...
//
// $Id: WordListOne.cc,v 1.23 2001/06/29 14:14:08 loic Exp $
//
...
WordReference WordListOne::Word(const String& bufferin, int exists /* = 0 */)
{
  WordReference wordRef(context);
  StringList fields(bufferin, "\t ");
  String* field = (String*)fields.Get_First();
  if(context->GetType().Normalize(*field) & WORD_NORMALIZE_NOTOK) {
    //
    // If the goal is to build a WordReference object that may not be
    // in the index, canonicalization failure is not a problem.
    //
    if(!exists)
      fprintf(stderr, "WordListOne::Word: cannot normalize word %s\n", (char*)*field);
  }
  String word = *field;
  unsigned int wordid;
  if(exists)
