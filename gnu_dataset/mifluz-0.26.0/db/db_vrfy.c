 ...
 *
 * $Id: db_vrfy.c,v 1.2 2014/04/17 20:27:28 sebdiaz Exp $
 */

#include "config.h"

#ifndef lint
static const char revid[] = "$Id: db_vrfy.c,v 1.2 2014/04/17 20:27:28 sebdiaz Exp $";
#endif /* not lint */
	...
	for (guess = DB_MAX_PGSIZE; guess >= DB_MIN_PGSIZE; guess >>= 1) {
		...
		for (i = 1; i <= 3; i++) {
			if ((ret = CDB___os_seek(dbenv, fhp, guess,
			    i, SSZ8(DBMETA, type), 0, DB_OS_SEEK_SET)) != 0)
				break;
