	...
	ssize_t *niop;
{
  ...
  case DB_IO_READ:
    if(db_io->pgno == PGNO_BASE_MD) {
      ret = CDB___os_io(dbenv, db_io, DB_IO_READ, ( size_t *)niop);
  ...
  case DB_IO_WRITE:
	...
	ret = CDB___os_io(dbenv, db_io, DB_IO_WRITE, (size_t *) niop);
     ...
     ssize_t *nrp;
{
  ...
  if((ret = CDB___os_read(dbenv, fhp, buff, buff_length, (size_t *)nrp)) != 0)
    goto err;

  if((size_t)*nrp != buff_length)
    goto err;
	...
	ssize_t *niop;
{
   ...
   * going anywhere.
   */
  if(ret || (size_t)*niop < db_io->pagesize)
    goto err;
  ...
  do {
	...
	goto err;
       ...
       */
      if((ret = CDB___os_io(dbenv, db_io, DB_IO_READ, (size_t*)niop)) != 0 ||
	 (size_t)*niop != db_io->pagesize) {
	ret = EIO;
	goto err;
      }
      /*
	...
	ssize_t *niop;
{
   ...
   */
  if((size_t)buffcmpr_length > (size_t)(DB_CMPR_PAGESIZE(db_io) * CMPR_MAX)) {
  ...
  do {
    size_t length = buffcmpr_length - (buffp - buffcmpr);
    size_t copy_length = (size_t)length > DB_CMPR_PAGESIZE(db_io) ? DB_CMPR_PAGESIZE(db_io) : length;
    /*
    ...
    buffp += copy_length;
    /* actual output  */
    if((ret = CDB___os_io(dbenv, db_io, DB_IO_WRITE, (size_t*)niop)) != 0 ||
       (size_t)*niop != db_io->pagesize) {
      ret = EIO;
      goto err;
    }
     ...
     int flags;
{
  ...
  if((ret = CDB___os_write(dbenv, fhp, buffer, pgsize, (size_t*)&count)) < 0) {
    CDB___db_err(dbenv, "CDB___memp_cmpr_create: write error at 0");
    return CDB___db_panic(dbenv, ret);
  }
  if((size_t)count != pgsize) {
     ...
     const char *path;
{
  ...
  if((ret = CDB___os_read(dbenv, &fh, (void*)&meta, sizeof(CMPRMETA), (size_t*)&count)) != 0) {
