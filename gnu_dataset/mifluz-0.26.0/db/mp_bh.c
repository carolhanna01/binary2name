...
#ifndef lint
static const char revid[] = "$Id: mp_bh.c,v 1.9 2014/04/17 20:27:30 sebdiaz Exp $";
#endif /* not lint */
	...
	int can_create;
{
	...
	if (F_ISSET(&dbmfp->fh, DB_FH_VALID)) {
		...
		if(F_ISSET(dbmfp, MP_CMPR)) {
		  ret = CDB___memp_cmpr(dbmfp, bhp, &db_io, DB_IO_READ, (ssize_t*)&nr);
		} else {
...
#endif /* DEBUG */
	if(F_ISSET(dbmfp, MP_CMPR)) {
	  ret = CDB___memp_cmpr(dbmfp, bhp, &db_io, DB_IO_WRITE, (ssize_t*)&nw);
	} else {
