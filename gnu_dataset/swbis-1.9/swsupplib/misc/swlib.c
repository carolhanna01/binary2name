...
static
ssize_t
synct_i_read_block(SYNCT * synct, int fd, char * buf)
{
	ssize_t ret;
	E_DEBUG2("fd=%d", fd);
	ret = uxfio_sfa_read(fd, buf, SWLIB_SYNCT_BLOCKSIZE);
	E_DEBUG("");
	if (ret != SWLIB_SYNCT_BLOCKSIZE) {
		fprintf(stderr, "%s: synct_i_read() error: ret=%d\n", swlib_utilname_get(), ret);
		if (ret < 0) return -1;
		return -2;
	}
	E_DEBUG("");
		...
		do {
			...
			if (bytes < 0) {
				...
				*/
				...
				if (obuf) {
					strob_memcpy_at(obuf, buf, (size_t)(bytes), (size_t)byteswritten);
				}
			}
			E_DEBUG("");
			byteswritten += bytes;
...
swlib_synct_read(SYNCT * synct, int fd, void * userbuf)
{
	...
	E_DEBUG2("fd = %d", fd);
	ret = synct_i_read_block(synct, fd, (char*)ibuf);
	...
	if (check_ret == 0) {
		...
		/* EOF found */
		/* read 1 more block which should be NUL
		   if so its the trailer block therefore
		   return 0 bytes to the user */
		E_DEBUG2("fd = %d", fd);
		ret = synct_i_read_block(synct, fd, (char*)tbuf);
		if (ret < 0) return -2;
...
swlib_apply_location(STROB * relocated_path, char * path, char * location, char * directory)
{
	...
	if (strlen(clean_directory) > 0) {
		prefix = strstr(path, clean_directory);
		if (prefix != NULL && prefix == path) {
			 path += strlen(clean_directory);
		}
	} else {
		;
	}
	strob_strcpy(relocated_path, location);
	swlib_unix_dircat(relocated_path, path);
	return;
}
