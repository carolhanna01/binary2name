...
debpsf_write_psf_buf(void * v_topsf, STROB * psf)
{
	...
	if (1 && 0 /*disabled*/) { /* debug and development code */
		if (control_buf)
			fprintf(stderr, "%s", strob_str(control_buf));
		if (da->Description)
			fprintf(stderr, "[%s]\n", strob_str(da->Description));
		if (da->Version)
			fprintf(stderr, "Version: [%s]\n", strob_str(da->Version));
		if (da->Version_revision)
			fprintf(stderr, "Upstream revision: [%s]\n", strob_str(da->Version_revision));
		if (da->Version_release)
			fprintf(stderr, "Debian revision: [%s]\n", strob_str(da->Version_release));
	}
