...
int
swi_afile_write_script_cases(SWI_SCRIPTS * scripts, STROB * buf)
{
	int i;
	char * lslash;
	char * name;
		...
		if (control_script) {
			...
			*lslash = '\0';
			...
			strob_sprintf(buf, STROB_DO_APPEND,
				...
				"\t\tcase \"$SW_CATALOG\" in\n"
				"\t\t\t/*) export SW_CONTROL_DIRECTORY=\"${SW_CATALOG}/${SWBIS_CATALOG_ENTRY}/%s/%s\"\n"
				"\t\t\t;;\n"
				"\t\t\t*) export SW_CONTROL_DIRECTORY=\"${SW_ROOT_DIRECTORY}/${SW_CATALOG}/${SWBIS_CATALOG_ENTRY}/%s/%s\"\n"
				"\t\t\t;;\n"
				"\t\tesac\n"
				"\t\t\tSWBCS_SCRIPT_NAME=\"%s\"\n"
				"\t\t\t;;\n",
