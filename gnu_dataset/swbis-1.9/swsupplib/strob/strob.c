...
strob_release(STROB * strb)
{
	...
	strb->str_ = (unsigned char*)NULL;
	strb->in_use_ = 1;
	strob_reopen(STROB_INITLENGTH + 1, strb);
	strob_close(strb);
#else
	swbis_free(strb);
#endif
...
strob_chr_index(STROB * strb, int index, int ch)
{
	...
	/*
		replaced with code below 2010-08-01
	if (index >= (int)strlen((unsigned char*)(strb->str_))) {
		memset(strb->str_ + strlen((unsigned char*)(strb->str_)),
				'\0', index - strlen((unsigned char*)(strb->str_)) + 1);
		*(strb->str_ + index + 1) = '\0';
	}
	*/

	if (index >= strb->length_) {
		memset(strb->str_ + strb->length_, '\0', index -  strb->length_ + 1);
		*(strb->str_ + index + 1) = '\0';
	}
...
strob_strncat(STROB * strb, char *str, size_t len)
{
	...
	ilen = (int)len;
	strb_ret = strob_reopen_if(strlen((char*)(strb->str_)) + ilen + 1, strb);
	if (!strb_ret) {
		return (char *)(NULL);
	}
