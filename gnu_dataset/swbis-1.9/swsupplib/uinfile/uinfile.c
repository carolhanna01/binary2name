...
LABEL_REREAD:
	...
	if (doarb == 0) {
		if (0) {
			...
			memcmp(xbuffer, (void *) UINFILE_MAGIC_gpg_sym,  strlen(UINFILE_MAGIC_gpg_sym)) == 0 ||
			memcmp(xbuffer, (void *) UINFILE_MAGIC_gpg_enc1,  strlen(UINFILE_MAGIC_gpg_enc1)) == 0 ||
			memcmp(xbuffer, (void *) UINFILE_MAGIC_gpg_enc2,  strlen(UINFILE_MAGIC_gpg_enc2)) == 0 ||
			0
		) {
			...
			if (memcmp(((unsigned char*)xbuffer)+deb_gz_offset, (void *)gzmagic, LENGTH_OF_MAGIC) != 0) {
				fprintf(stderr, "%s: gz magic not found for control.tar.gz\n", swlib_utilname_get());
				return -1;
			}
			...
			if (
				...
				while ((name=swvarfs_get_next_dirent(swvarfs, &st)) != NULL && strlen(name)) {
					...
					if (strcmp(name, UINFILE_MAGIC_slack "/") == 0) {
			...
			*/
			...
			if (taru_tape_buffered_read(refd,
				xbuffer + peeklen, 512 - peeklen) != (512 - peeklen)) return -1;
	...
	if (zcat_command && (*uinformat)->ztypeM == UINFILE_COMPRESSED_Z) {
		...
		if (memcmp(((char*)(xbuffer)) + deb_gz_offset,
			 ...
			 */
			fprintf(stderr, "%s: internal error %s: at line %d\n", swlib_utilname_get(), __FILE__, __LINE__);
			return -1;
		} 
