...
/*
 * Copyright (C) 1999,2007,2008  James H. Lowe, Jr. 
 * 
...
{
	fprintf(fp,  "%s",
	"Copyright (C) 2003,2004,2005,2007 Jim Lowe\n"
...
void  usage (char * progname, FILE * fp) {
    ...
    fprintf (fp, "   --exclude-system-dirs  exclude all system directories, such as /usr, /usr/bin,...\n");
    ...
    fprintf (fp, "   -H format     archive format: ustar, newc, crc, odc\n");
...
add_system_dirs(STRAR * exclude_list)
{
	...
	strar_add(exclude_list, "/etc/init.d");
	...
	strar_add(exclude_list, "/usr/info");
	strar_add(exclude_list, "/usr/doc");
	strar_add(exclude_list, "/usr/X11R6");
	strar_add(exclude_list, "/usr/man/man1");
	strar_add(exclude_list, "/usr/man/man2");
	strar_add(exclude_list, "/usr/man/man3");
	strar_add(exclude_list, "/usr/man/man4");
	strar_add(exclude_list, "/usr/man/man5");
	strar_add(exclude_list, "/usr/man/man6");
	strar_add(exclude_list, "/usr/man/man7");
	strar_add(exclude_list, "/usr/man/man8");
	...
	strar_add(exclude_list, "/libexec");
}
...
write_plain_source_tarball(TOPSF * topsf)
{
	...
	E_DEBUG("");
	xformat_set_ofd(xformat, STDOUT_FILENO);
...
write_deb(TOPSF * topsf)
{
	...
	ret = xformat_open_archive_by_fd(data_xformat, data_fd,
		UINFILE_DETECT_OTARFORCE|UINFILE_DETECT_DEB_CONTEXT, (mode_t)(0));
	xformat_set_ofd(data_xformat, STDOUT_FILENO);
	...
	*format='\0';
	...
	while (1)
	      	 ...
	      	 do_deb_control = 1;
		{
			...
			if (debfd < 0) exit(1);
			ret = swlib_pipe_pump(STDOUT_FILENO, debfd);
			uxfio_close(debfd);
	      	 ...
	      	 do_deb_data = 1;
		{
			...
			fd = ret;
			ret = swlib_pipe_pump(STDOUT_FILENO, fd);
			uxfio_close(fd);
			uinfile_close(uinformat);
			if (ret < 0)
				exit(1);
