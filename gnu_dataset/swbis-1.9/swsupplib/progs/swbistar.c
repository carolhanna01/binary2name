	...
	)
{
	...
	if (verbose > 0) {
		swlib_writef(ofd, wf, "%-12s[", fieldname);
	} else {
		swlib_writef(ofd, wf, "%s:=[", fieldname);
	}	
...
static
int
write_header_decode_format(STROB * wf, int ofd, char * buf, int buflen, int v)
{
	char * p;
	...
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_name, THB_FL_name, "name");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_mode, THB_FL_mode, "mode");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_uid, THB_FL_uid, "uid");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_gid, THB_FL_gid, "gid");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_size, THB_FL_size, "size");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_mtime, THB_FL_mtime, "mtime");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_chksum, THB_FL_chksum, "chksum");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_typeflag, THB_FL_typeflag, "typeflag");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_linkname, THB_FL_linkname, "linkname");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_magic, THB_FL_magic, "magic");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_version, THB_FL_version, "version");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_uname, THB_FL_uname, "uname");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_gname, THB_FL_gname, "gname");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_devmajor, THB_FL_devmajor, "devmajor");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_devminor, THB_FL_devminor, "devminor");
	write_field(v, wf, ofd, paxSum, Mchksum, paxheader , THB_BO_prefix, THB_FL_prefix, "prefix");
...
static
int
decode_filter(XFORMAT * package, int verbose)
{
	int nullfd;
	char * name;
	int ifd;
	...
	while ((ret = xformat_read_header(package)) > 0) {
		...
		write_header_decode_format(tmp, STDOUT_FILENO, extended_header, tarheader_length, verbose);
...
main (int argc, char ** argv ) {
	...
	while (1)
           {
             ...
             c = ugetopt_long (argc, argv, "tvaxLFXDH:", long_options, &option_index);
             if (c == -1)
                 break;
	       ...
	       case 159:
			{
				...
				ETAR * etar;
				etar = etar_open(taru);
				devno = swlib_atoul(optarg, (int *)NULL);	
	...
	if (dumpmode) {
			ret = decode_filter(xformat, verbose);
			exit(ret);
	}
