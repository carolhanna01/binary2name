...
check_files_by_md5() {
	...
	fsum=`find "${package_dir}" | ${SED} -e 's@/$@@' | ${SORT} | ${MD5SUM}`
	# OLD slow way:  fsum=`${TAR} cf - ${package_dir} | ${TAR} tf - | ${SED} -e 's@/$@@' | ${SORT} | ${MD5SUM}`
	case "$csum" in
		"") return 1
		;;
	esac
...
create_archive_stream() {
	...
	case "$tarcreateutil" in
		"$TAR")
			...
			# --exclude "${package_dir}"/catalog/\* 
			"${TAR}" cf - ${TAR_FORMAT_OPTIONS} --files-from=- --no-recursion \
