				...
				while(
					...
					if (compare_status != 0) {
						...
						swc_stderr_fd2_set(G, STDERR_FILENO);
							sw_l_msg(G, SWC_VERBOSE_1, "%s at sig_level=%s for %s: status=1\n",
							swevent_code(SW_SOC_INTEGRITY_NOT_CONFIRMED), wopt_sig_level, sc->sqM->swspec_stringM);
						swc_stderr_fd2_restore(G);
					} else {
						swc_stderr_fd2_set(G, STDERR_FILENO);
						sw_l_msg(G, SWC_VERBOSE_2, "%s at sig_level=%s for %s: status=0\n",
							swevent_code(SW_SOC_INTEGRITY_CONFIRMED), wopt_sig_level, sc->sqM->swspec_stringM);
						swc_stderr_fd2_restore(G);
					}
				}  /* loop over responses */
				E_DEBUG("");
