...
public: // ===============================================================================
	...
	processExcludes(swDefinitionFile* psf, char * value) {
		...
		//
		if (source && strlen(source) && source[strlen(source)-1] == '/') {
			//
			...
			//
			// do not augment with .../* to exclude contents
			// if indeed the source is a directory
			//
			;
		}
		...
		while ((swdef=psf->swdeffile_get_pointer_from_index(i++)) != static_cast<swDefinition*>(NULL)) {
			...
			// if (fnmatch(strob_str(excl_pat), mdef_source, 0) == 0) {
			if (process_fnmatch_list(pat_list, mdef_source) == 0) {
				//
				// Got match
				//
				...
				if (first_match == 0 && *type == 'd' && (directoryDestActive()==0 || does_have_trailing_slash)) {
...
private: // ================================================================
	...
	process_fnmatch_list(STRAR * pat_list, char * mdef_source) {
		...
		SWEXTDEF_DEBUG2("mdef_source is [%s]", mdef_source);
		ret = 1;
		i = 0;
		pat = strar_get(pat_list, i++);
		while(pat && ret) {
			SWEXTDEF_DEBUG3("pat[%d] is [%s]", i-1, pat);
			//
			// Check for the case
			...
			if (	pat &&
				...
				*(pat+strlen(pat)-1) == '/' &&
				...
				SWEXTDEF_DEBUG("Got special match 0001");
				ret = 0;
				break;
			}
			if (
			...
			SWEXTDEF_DEBUG3("---- fnmatch(%s, %s)", pat, mdef_source);
		...
		for (i=0; i<lastE; i++) {
			...
			switch(i) {
				...
				case gidE:
					...
					break;
				case ownerE:
					if (taru_get_user_by_uid(getuid(), usernamebuf) == 0) {
					...
					break;
				case groupE:
					if (taru_get_group_by_gid(getgid(), usernamebuf) == 0) {
