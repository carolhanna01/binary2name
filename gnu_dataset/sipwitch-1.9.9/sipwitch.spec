...
%package devel
Requires: ucommon-devel%{?_isa} >= 6.3.1
Requires: %{libname}%{?_isa} = %{version}-%{release}
...
%{_bindir}/sipquery
%{_bindir}/sipcontrol
%attr(0755,root,root) %{_bindir}/sippasswd
%dir %{_libdir}/sipwitch
%config(noreplace) %{_sysconfdir}/logrotate.d/sipwitch
%attr(0644,root,root) %{_prefix}/lib/systemd/system/sipwitch.service
...
%pre
%service_add_pre sipwitch.service

%post
%service_add_post sipwitch.service
%{__chmod} 04755 %{_bindir}/sippasswd
%set_permissions /usr/bin/sippasswd

%preun
%service_del_preun sipwitch.service

%postun
%service_del_postun sipwitch.service
