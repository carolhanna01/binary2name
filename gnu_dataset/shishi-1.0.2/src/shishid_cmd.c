...
void init_args_info(struct gengetopt_args_info *args_info)
{
  ...
  args_info->no_tls_help = gengetopt_args_info_help[6] ;
  args_info->x509cafile_help = gengetopt_args_info_help[7] ;
  args_info->x509certfile_help = gengetopt_args_info_help[8] ;
  args_info->x509crlfile_help = gengetopt_args_info_help[9] ;
  args_info->x509keyfile_help = gengetopt_args_info_help[10] ;
  args_info->resume_limit_help = gengetopt_args_info_help[11] ;
  args_info->configuration_file_help = gengetopt_args_info_help[13] ;
  args_info->verbose_help = gengetopt_args_info_help[14] ;
  args_info->verbose_min = 0;
  args_info->verbose_max = 0;
  args_info->quiet_help = gengetopt_args_info_help[15] ;
...
cmdline_parser_dump(FILE *outfile, struct gengetopt_args_info *args_info)
{
  ...
  if (args_info->no_tls_given)
    write_into_file(outfile, "no-tls", 0, 0 );
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 'V':	/* Print version and exit.  */
          ...
          break;
        case 'l':	/* Sockets to listen for queries on.  Family is `IPv4' or `IPv6', if absent the family is decided by gethostbyname(ADDR). An address of `*' indicates all addresses on the local host. The default is `*:kerberos/udp, *:kerberos/tcp'..  */
...
.  */
        ...
        case 0:	/* Long option with no short option */
          /* Disable TLS support.  */
          if (strcmp (long_options[option_index].name, "no-tls") == 0)
          {
          
          
            if (update_arg((void *)&(args_info->no_tls_flag), 0, &(args_info->no_tls_given),
                &(local_args_info.no_tls_given), optarg, 0, 0, ARG_FLAG,
                check_ambiguity, override, 1, 0, "no-tls", '-',
                additional_error))
              goto failure;
          
          }
          /* X.509 certificate authorities used to verify client certificates, in PEM format..  */
          else if (strcmp (long_options[option_index].name, "x509cafile") == 0)
          {
