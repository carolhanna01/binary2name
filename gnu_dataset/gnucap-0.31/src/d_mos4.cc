/* $Id: d_mos4.cc,v 21.14 2002/03/26 09:20:25 al Exp $ -*- C++ -*-
  ...
  :SDP_MOS_BASE(cc)
{
  ...
  etaB = m->etaB(L, W);
  etaD = m->etaD(L, W);
  mobZero = m->mobZero(L, W);
  mobZeroB = m->mobZeroB(L, W);
  mobVdd = m->mobVdd(L, W);
  mobVddB = m->mobVddB(L, W);
  mobVddD = m->mobVddD(L, W);
  ugs = m->ugs(L, W);
  ugsB = m->ugsB(L, W);
  uds = m->uds(L, W);
  udsB = m->udsB(L, W);
