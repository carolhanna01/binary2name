/*$Id: e_compon.cc,v 21.14 2002/03/26 09:20:25 al Exp $ -*- C++ -*-
...
const MODEL_CARD* COMMON_COMPONENT::attach_model(const COMPONENT* d)const
{
  {if (modelname() != "") {
	...
	mi = find_ptr(root_model_list.begin(), root_model_list.end(),
		      extended_name);
	{if (mi == root_model_list.end()) {
	  {if (bin_count <= 1) {
...
int COMPONENT::parse_nodes(CS& cmd, int max_nodes, int min_nodes, int start)
{
  ...
  if (start < max_nodes) {
    ...
    for (int ii = start;  ii < max_nodes;  ++ii) {
	...
	{if (ii < min_nodes) {
	  ...
	  _n[ii].t = _n[ii].e = 0;
	}else if (ii == min_nodes){
	  _n[ii].t = _n[ii].e = 0; // bjt hack
	}}
      }
    }
 ...
 */
void COMPONENT::printnodes(OMSTREAM& where, int num_nodes, int start)const
{
  for (int ii = start;  ii < num_nodes && ii < port_count();  ++ii) {
    where << "  " << _n[ii];
  }
}
/*--------------------------------------------------------------------------*/
