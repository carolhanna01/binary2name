/*$Id: d_logic.cc,v 21.14 2002/03/26 09:20:25 al Exp $ -*- C++ -*-
...
{
  parse_Label(cmd);
  int incount = parse_nodes(cmd, max_nodes(), BEGIN_IN);
  if (incount <= BEGIN_IN) {
    untested();
    incount = BEGIN_IN +1;
...
void DEV_LOGIC::tr_accept()
{
  ...
  NODE* ns[PORTS_PER_GATE];
  {
    ...
    for (int ii = BEGIN_IN;  ii < num_nodes();  ++ii) {
      _n[ii]->to_logic(m);
      ns[ii] = &*(_n[ii]);
