...
void myMove(int i)
{
  ...
  if (mav_SMSIntersectBBAll(mav_win_current, bb, tmp)) {
    *(mav_win_current->vp)= orig; /* Collision occurred, so use original view parameters */
...
int myNav(MAV_object *o, MAV_mouseEvent *me)
{
  if (me->movement==MAV_PRESSED)
  {
     ...
     mav_frameFn0Add(myMoveFn[me->button], NULL);
  }
  else
  {
     /* Stop executing myMove function */
     mav_frameFn0Rmv(myMoveFn[me->button], NULL);
  }

  /* Also pass event onto ap defined mouse event callback fn */
  return MAV_FALSE;
}



/* Code for the overview window */
...
{
  /* Display users BB in overview window */
  mav_BBDisplay(ov, bb);
}
