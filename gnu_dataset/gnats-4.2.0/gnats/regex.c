/* Extended regular expression matching and search library, version
   ...
   Copyright (C) 1993,94,95,96,97,98,99,2000,07 Free Software Foundation, Inc.
...
static re_wctype_t re_wctype (re_char *);
...
static boolean re_iswctype (int, re_wctype_t);
...
/* Return a bit-pattern to use in the range-table bits to match multibyte
   chars of class CC.  */
static int re_wctype_to_bit (re_wctype_t);
...
static unsigned char * skip_noops (unsigned char *p, unsigned char *pend);
static int mutually_exclusive_p (struct re_pattern_buffer *bufp,
				 unsigned char *p1, unsigned char
				 *p2);
#ifndef emacs	/* Emacs never uses this.  */
int re_match (struct re_pattern_buffer *bufp, const char *string, int
	      size, int pos, struct re_registers *regs);
#endif /* not emacs */
...
static int re_match_2_internal ( struct re_pattern_buffer *bufp,
				 ...
				 struct re_registers *regs,
				 int stop);
static int bcmp_translate ( re_char *s1, re_char *s2, register int
			    len, RE_TRANSLATE_TYPE translate, const
			    int multibyte);
const char * re_compile_pattern (const char *pattern, size_t length,
				 struct re_pattern_buffer *bufp);

/* weak functions re_comp, re_exec */
char *
# ifdef _LIBC
weak_function
# endif
...
int regcomp (regex_t *__restrict preg, const char *__restrict pattern,
	     int cflags);
int regexec (const regex_t *__restrict preg, const char *__restrict
