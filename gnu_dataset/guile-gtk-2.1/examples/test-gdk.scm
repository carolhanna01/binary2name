...
;; `truncate' avoids getting a fraction in guile 1.8
(define (div n d)
  (truncate (quotient n d)))

;; round the flonum or fraction X to an exact integer
...
(define (arc-drawer-new width height start-adj extent-adj)
  (let ((widget (gtk-drawing-area-new))
	...
	(define (draw-arc-handle angle)
	  ...
	  (let ((x (round-exact (* 0.5 w (cos (->rad angle)))))
		(y (round-exact (* -0.5 h (sin (->rad angle))))))
    ...
    (define (update)
      (set! start (round-exact (* (gtk-adjustment-value start-adj) 64)))
      (set! extent (round-exact (* (gtk-adjustment-value extent-adj) 64)))
      (cond (window
