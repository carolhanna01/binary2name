...
(define-test create-clist "clist"
  (let* ((titles #("auto resize" "not resizeable" "max width 100" "min width 50"
    ...
    (set! button (hack-gtk-radio-menu-item-new-with-label-from-widget #f "Single"))
...
(define-test create-radio-buttons "radio buttons"
  (let ((group #f))
    (define (radio-maker label)
      (set! group (hack-gtk-radio-menu-item-new-with-label-from-widget group label))
      group)
...
(define-test create-pixmap "pixmap"
  (let* ((button (gtk-button-new))
	 (pixmap (gtk-pixmap-new-from-file "test.xpm" button))
...
(define (make-menu depth)
  (if (< depth 1)
	...
	(do ((i 0 (1+ i))
	     (j 1 (1+ j)))
	    ((= i 6))
	  (let ((menuitem (hack-gtk-radio-menu-item-new-with-label-from-widget
			   group
