...
static char rcsid[] = "$Id: rpc_parse.c,v 1.1.1.1 2005/10/28 14:46:46 k0ro Exp $";
#endif
...
static void def_struct(), def_program(), def_enum(), def_const(),
	...
	definition *defp;
{
	...
	do {
		...
		do {
			...
			get_type(&plist->res_prefix, &plist->res_type, &dummy, DEF_PROGRAM);
			...
			scan(TOK_LPAREN, &tok);
			get_type(&plist->arg_prefix, &plist->arg_type, &dummy, DEF_PROGRAM);
	...
	defkind dkind;
{
	...
	get_type(&dec->prefix, &dec->type, &dec->storage, dkind);
	dec->rel = REL_ALIAS;
	dec->pstorage[0] = '\0';
...
static void
get_type(prefixp, typep, stg, dkind)
	char **prefixp;
	char **typep;
	int	*stg;
	defkind dkind;
