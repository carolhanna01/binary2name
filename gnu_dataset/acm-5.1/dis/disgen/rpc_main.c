...
static char rcsid[] = "$Id: rpc_main.c,v 1.1.1.1 2005/10/28 14:46:46 k0ro Exp $";
#endif
...
#ifdef WIN32
static char CPP[] = "cl";
static char CPPFLAGS[] = "/E";
#else
#ifndef CPPEXEC
#define CPPEXEC "/lib/cpp"
#endif
static char CPP[] = CPPEXEC;
#ifdef __hpux
static char CPPFLAGS[] = "-PC";
#else
...
static void h_output(), c_output(), s_output(), l_output(), do_registers();
static int parseargs();
extern void reinitialize(void), crash(), write_programs();
...
#ifdef WIN32
	if (cmd.cflag) {
		c_output(cmd.infile, "/D \"RPC_XDR\"", !EXTEND, cmd.outfile);
	} else if (cmd.hflag) {
		h_output(cmd.infile, "/D \"RPC_HDR\"", !EXTEND, cmd.outfile);
	} else if (cmd.lflag) {
		l_output(cmd.infile, "/D \"RPC_CLNT\"", !EXTEND, cmd.outfile);
	} else if (cmd.sflag || cmd.mflag) {
		s_output(argc, argv, cmd.infile, "/D \"RPC_SVC\"", !EXTEND,
			 cmd.outfile, cmd.mflag);
	} else {
		c_output(cmd.infile, "/D \"RPC_XDR\"", EXTEND, "_xdr.c");
		reinitialize();
		h_output(cmd.infile, "/D \"RPC_HDR\"", EXTEND, ".h");
		reinitialize();
		l_output(cmd.infile, "/D \"RPC_CLNT\"", EXTEND, "_clnt.c");
		reinitialize();
		s_output(allc, allv, cmd.infile, "/D \"RPC_SVC\"", EXTEND,
			 "_svc.c", cmd.mflag);
	}
#else
	if (cmd.cflag) {
	...
	char *define;
{
	...
	_close (1);
	_dup2 (pd[1], 1);
	s = _spawnlp (_P_NOWAIT, CPP, CPP, CPPFLAGS, "/C",
		"/nologo", define, infile, NULL);
	_dup2 (my_stdout, 1);
	_close (my_stdout);
	if (s == -1) {
		perror("_spawnl");
