...
static char rcsid[] = "$Id: rpc_cout.c,v 1.1.1.1 2005/10/28 14:46:46 k0ro Exp $";
#endif
...
static void print_header(), print_trailer(), space(), emit_enum(),
	...
	definition *def;
{
	...
	for (dl = def->def.st.decls; dl != NULL; dl = dl->next) {
		...
		if (dl->decl.storage == -1) {
		    d = (definition *) FINDVAL(defined, dl->decl.type, findtype);
		    if (d) {
			if (d->def_kind == DEF_TYPEDEF) {
			...
			if (szof != -1) {
			    if (dl->decl.rel == REL_ARRAY2) {
			/*	size += szof; */
				sprintf (buf , "( %s * %d )", dl->decl.array_max, szof);
			    }
			    else {
				size += szof;
				sprintf (buf, "%d", szof);
