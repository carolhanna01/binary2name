...
function get_scores_arr ($db_name="", $id=0, $period="", $date_clause)
{
  ...
  while (list($period,$ids_names_num) = each($period_idsNames_ass))
  {
    ...
    while (list(,$id_name_arr) = each($ids_names_num))
    {
      ...
      $testidDate_score_ass=
                  get_scores_arr($db_name,$id,$period,$date_clause);
      for ($i=0; $i<sizeof($testid_date_num); $i++)
      {
...
function set_scores_makeup($db_name,$id,$test_id,$date,$period,$color)
{
  $makeup = "";
  $query = "select makeup from $db_name.scores where test_id = $test_id
  ...
  if (mysql_num_rows($result) == 0)
  {
    $makeup = "y";
    $query="insert into
             $db_name.scores(id,test_id,date,period,makeup,score)
  ...
  $query="update $db_name.scores set makeup='$makeup' where test_id=$test_id
          and date='$date' and id=$id and period='$period'";
  $result = mysql_query($query);
...
function get_test_form 
       ($id=0,$testid_date="",$score="", $makeup="n", $period="")
{
...
EOQ;
  ...
  {
    $page = 
"/teachers/tests_admin/new_passwords.php?id=$id&period=$period";
    $window_description =
...
$comment = "Please click on the grade to be made up.";
title_comment_div_noend($page_title, $comment);
...
{
  set_scores_makeup
($db_name,$id_makeup,$testid_makeup,$date_makeup,$period_makeup,$color);
}

$search_r = glue_search_inputs($_POST);
$date_query = get_date_query($search_r);
show_periods_scores_table ($db_name, $spanish_query, $date_query);
