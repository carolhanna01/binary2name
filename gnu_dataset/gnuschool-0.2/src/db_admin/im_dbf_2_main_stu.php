...
function trim_rr_2_r($column_rr)
{
  $new_r = array();
  $size = sizeof($column_rr);
  for ($i=0; $i< $size; $i++)
  { 
    $name = $column_rr[$i]['name'];
    $val = trim($name);
...
$tag_title = "gnuschool.org Edit Database With dbase File";
$page_title = "Edit database with a dbase file";
...
if (!$yes_input)
{
  $comment = "Edit the database using the default dbf file.";
  title_comment_div_noend($page_title, $comment);
  echo "<b>Notes:
        <ul>
        <LI>The dbf file must have at
...
$comment = "Attempting to edit the database using the default dbf file...";
title_comment_div_noend($page_title, $comment);
...
$dbf_header_rr = dbase_get_header_info($dbf);
if ($dbf_header_rr === FALSE)
{
  echo "<b>Error obtaining header information from the dbase file!</b>";
  echo "<br><br>";
  back_2_parentA("setup");
  end_html_exit();
}
...
{
  echo "<b>Error obtaining record numbers from the dbase file!</b>";
  echo "<br><br>";
  back_2_parentA("setup");
  end_html_exit();
}
...
$line_col_r = trim_rr_2_r($dbf_header_rr);
...
if($order_r === FALSE)
{
  echo "<b> The default column names do not match with the dbase file 
...
{
  echo "<b> The default column names match with the dbase file column 
...
$edited = 0;
for ($w=1; $w< $record_numbers + 1; $w++)
{
  $row = dbase_get_record_with_names($dbf, $w);
  ...
  else
  {
    ...
    {
      $fname = $row["$col_fname_val"];
      $mname = $row["$col_mname_val"];
      $lname = $row["$col_lname_val"];
      $fname = trim($fname);
      $mname = trim($mname);
      $lname = trim($lname);
    ...
    if ($kt_hr && $ok_line)
    {
      $hr = $row["$col_hr_val"];
      $hr = trim($hr);
      $temp = is_valid_file_name($hr);
      if (!$temp)
      { 
          echo "<b>SKIPPING: HOMEROOM $hr is not a valid name
                .</b><br><br>";
          $ok_line = 0;
      }
    }
