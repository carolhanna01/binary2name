...
// print "spanish_query=$spanish_query <br><br>";
  ...
  { 
    ...
    $query1="select db_name from main.teachers where hr='$next_hr_num'";
    $result1 = mysql_query($query1);
    $row1 = mysql_fetch_row($result1);
    $id_name_arr['hr_db'] = $row1[0];
...
function get_attendance_arr($db_name="", 
            $id=0,$date_clause="",$period="",$hr_db="",$today_date="0")
{
  $date_time_in = array();
                                  ...
                                  $today_date)
{
  ...
  $cell_data = tDataSource("Homeroom $period attendance", "center", "",
                            sizeof($title_arr) +1);
  ...
  if($today_date !== "0") $cell_data .= tDataSource($today_date, "right");
  echo rowPrint($cell_data);
}
             ...
             $in_by="", $today_date="0")
{
  ...
  while (list($period,$ids_names_num) = each($period_idsNames_ass))
  {
    ...
    show_table_column_titles($dates_arr, $period, $today_date);
    while (list(,$id_name_arr) = each($ids_names_num))
    {
      ...
      $atten_in_ass=get_attendance_arr($db_name,
                   $id,$date_clause,$period,$hr_db,$today_date);
...
$tag_title = "gnuschool.com Absent search";
...
$alldate = getdate();
$date = "$alldate[year]-$alldate[mon]-$alldate[mday]";
$time = "$date[hours]:$date[minutes]:$date[seconds]";

$page_title="Absent search&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
...
$page_title="Attendance records&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
            &nbsp&nbsp&nbsp&nbsp(this page was opened $date at $time)";
