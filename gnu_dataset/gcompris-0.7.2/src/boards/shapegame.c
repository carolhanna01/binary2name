 ...
 *
 * Time-stamp: <2001/09/12 23:45:42 bruno>
 *
...
{
  char *name, *pixmapfile, *targetfile, *cx, *cy, *cd, *czoomx, *czoomy, *cposition, *ctype, *justification;
...
read_xml_file(char *fname)
{
  ...
  if(/* if there is no root element */
     ...
     g_strcasecmp(doc->root->name,"ShapeGame")!=0) {
    xmlFreeDoc(doc);
    return FALSE;
  }
...
write_xml_file(char *fname)
{
  ...
  doc->root = xmlNewDocNode(doc, NULL, "ShapeGame", NULL);
  
  /* loop through all our shapes */
  for(list = shape_list; list != NULL; list = list->next) {
    Shape *shape = list->data;
    if(shape->type!=SHAPE_ICON)
      write_shape_to_xml(doc->root, shape);
  }
