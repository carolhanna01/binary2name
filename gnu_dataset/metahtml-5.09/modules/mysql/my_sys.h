...
#ifdef THREAD
extern int *_my_errno(void) __attribute__ ((const));
#define my_errno  (*_my_errno())
#else
...
extern int NEAR my_umask,		/* Default creation mask  */
	   ...
	   NEAR mysys_uses_curses;
extern long lCurMemory,lMaxMemory;	/* from safemalloc */

extern ulong	my_default_record_cache_size;
extern int NEAR my_disable_locking,NEAR my_disable_async_io;
...
extern int open_cacheed_file(IO_CACHE *cache,const char *prefix,
			     uint cache_size);
...
extern int find_type(my_string x,TYPELIB *typelib,int full_name);
